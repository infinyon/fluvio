...
impl LeaderStreams {
    ...
    #[allow(dead_code)]
    async fn watch_changes(self: Arc<Self>, leader_id: SpuId,mut stream: KfTcpStreamSplitStream) -> Result<(),IoError>{
        ...
        while let Some(result) = stream.next().await {
            ...
            debug!("fetch stream: received from spu: {},ack bytes: {} ",leader_id,bytes.len());
            // decode request
            let mut src = Cursor::new(&bytes);
            let _request = RequestMessage::<InternalApiRequest,InternalKafkaApiEnum>::decode_from(&mut src);
...
#[allow(dead_code)]
 async fn send_ack(leader_id: SpuId, mut sink: WriteGuard<SpuId,KfTcpStreamSplitSink>) -> Result<(),IoError> {
