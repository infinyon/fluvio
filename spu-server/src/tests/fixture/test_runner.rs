...
impl <T>SpuTestRunner<T> where T: SpuTest + Send + Sync + 'static {
    ...
    pub async fn run(client_id: String, test: T) -> Result<(),KfSocketError> {
        ...
        arc_runner.set_sc_ctx(ScServerCtx { ctx: sc_server_ctx, sender});
    ...
    async fn run_test(self: Arc<Self>) {
        ...
        self.test().main_test(self.clone()).await.expect("test should run");
    ...
    async fn terminate_server(&self) {
        ...
        for i in 0..self.spu_server_specs.len() {
            ...
            sender
                .send(true)
                .await
                .expect("spu shutdown should work");
        ...
        if let Some(mut sc_sender) = self.sc_sender() {
            ...
            sc_sender
                .send(true)
                .await
                .expect("sc shutdown should work");
        }
    ...
    pub fn replica_ids(&self) -> Vec<SpuId> {
        self.spu_server_specs.iter().map(|follower| follower.spec().id).collect()
    } 
    ...
    pub async fn send_metadata_to_spu<'a>(&'a self,sink: &'a mut KfSink,target_spu: SpuId) -> Result<(),KfSocketError> {
