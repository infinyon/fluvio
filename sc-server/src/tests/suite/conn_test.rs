...
use types::SpuId;
use metadata::spu::SpuResolution;
...
impl ScTest for SimpleInternalTest {
    ...
    fn main_test(&self,runner: Arc<ScTestRunner<SimpleInternalTest>>) -> Self::ResponseFuture {
        ...
        async move {
            ...
            debug!("spu server: {} is up, let's make sure sc and spu done it's work",BASE_ID);
...
async fn connection_test() -> Result<(), KfSocketError> {
    ...
    ScTestRunner::run("connection test".to_owned(),test).await.expect("test runner should not failer");
    Ok(())
}
