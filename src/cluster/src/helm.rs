...
impl HelmClient {
    ...
    pub fn new() -> Result<Self, IoError> {
        ...
        let out_str = String::from_utf8(output.stdout)
            .map_err(|e| {
                IoError::new(ErrorKind::InvalidData, "failed to parse helm output as string")
            })?;
        ...
        if !out_str.contains("version") {
            return Err(IoError::new(ErrorKind::InvalidData, "failed to get helm version"));
        }
    ...
    pub fn install(&self, name: &str, chart: &str, opts: &[&str]) -> Result<(), IoError> {
        let mut sets = vec![];
        for opt in opts {
            sets.push("--set");
            sets.push(opt);
        ...
        let status = Command::new("helm")
            ...
            .status()?;
        if !status.success() {
            return Err(IoError::new(ErrorKind::Other, "helm install failed"));
        }
        Ok(())
    }
    ...
    pub fn repo_add(&self, chart: &str, location: &str) -> Result<(), IoError> {
        let status = Command::new("helm")
            ...
            .status()?;
        if !status.success() {
            return Err(IoError::new(ErrorKind::Other, "helm repo add failed"));
        }
        Ok(())
    }
    ...
    pub fn repo_update(&self) -> Result<(), IoError> {
        let status = Command::new("helm")
            ...
            .status()?;
        if !status.success() {
            return Err(IoError::new(ErrorKind::Other, "helm repo update failed"));
        }
        Ok(())
    }
    ...
    pub fn search_repo(&self, chart: &str) -> Result<Vec<Chart>, IoError> {
        ...
        serde_json::from_slice(&output.stdout)
            .map_err(|_| {
                IoError::new(ErrorKind::InvalidData, "failed to parse helm chart versions")
            })
    }
    ...
    pub fn chart_version_exists(&self, name: &str, version: &str) -> Result<bool, IoError> {
        ...
        let count = versions.iter()
            .filter(|chart| chart.name == name && chart.version == version)
            .count();
        Ok(count > 0)
    }

    /// Lists installed charts matching the given chart name
    pub fn list_charts(&self, chart: &str) -> Result<Vec<InstalledChart>, IoError> {
        let output = Command::new("helm")
            .args(&["list", "--filter", chart, "--output", "json"])
            .print()
            .output()?;
...
mod tests {
    ...
    #[test]
    fn test_helm_client_check() {
        HelmClient::new().expect("should find helm client");
    }
}
