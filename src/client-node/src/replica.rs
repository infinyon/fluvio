...
impl JsReplicaLeader {
    ...
    ///                 }
    #[node_bindgen(mt)]
    fn consume<F: Fn(String, String) + 'static + Send + Sync >(&self,offset_option: JsObject, cb: F) -> Result<(),NjError>
     {
        ...
        let offset = if let Ok(offset_string) = offset_option.as_value::<String>() {
            ...
            debug!("offset property founded");
            let offset_string = offset_property.as_value::<String>()?;
            offset_from(offset_string)?
...
async fn consume_inner<F: Fn(String, String)>(leader: SharedReplicaLeader, offset: FetchOffset, option: FetchLogOption, cb: F) -> Result<(),NjError>  {
...
fn offset_from(value: String) -> Result<FetchOffset,NjError> {
    ...
    if value == "earliest" {
        ...
        return Err(NjError::Other(format!("invalid option: {}, valid values are: earliest/latest",value)))
    }
