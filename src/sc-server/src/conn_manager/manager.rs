...
impl Default for ConnManager {
    fn default() -> Self {
        Self::new(SpuLocalStore::new_shared(),PartitionLocalStore::new_shared())
    }
}
...
impl ConnManager {
    ...
    pub fn new_with_local_stores(local_stores: ShareLocalStores) -> Self {
        Self::new(local_stores.spus().clone(),local_stores.partitions().clone())
    }
    ...
    fn inner_add_spu(&self,spu: &SpuSpec) {
        ...
        if let Some(conn) = self.sinks.get_sink(&spu.id) {
            ...
            warn!("unexpected socket entry found for Spu({}). clearing ", spu.id);
    ...
    async fn refresh_partition(&self,key: ReplicaKey,spec: PartitionSpec) {
        ...
        replica_msgs.push(ReplicaMsg::update(
            Replica::new(
                key,
                spec.leader,
                spec.replicas.clone())));
        ...
        message
            .get_mut_header()
            .set_client_id("controller");

        for spu in spec.replicas {
            debug!("sending replica: {} to spu: {}",message.request.decode_request(),spu);
            match self.send_msg(&spu, &message).await {
                Ok(status) => {
                    if !status {
                         trace!("unable to send partition: {} to offline spu: {}",spec.leader,spu);
                    }
                },
                Err(err) => warn!("error {} sending partition: {} to spu: {}",err,spec.leader,spu)
            }
        }
    }
    ...
    async fn send_update_all_to_spu<'a>(
        &'a self,
        spu: &'a SpuKV,
    ) -> Result<(), ScServerError> {
        let spu_specs = self.spu_store.all_values()
            .into_iter()
    ...
    async fn send_msg_to_all_live_spus(&self,msgs: Vec<SpuMsg>) {
        ...
        debug!("trying to send SPU spec to active Spu: {}",online_spus.len());
        for live_spu in online_spus {
            if let Err(err) = self.send_update_spu_msg_request(&live_spu,msgs.clone()).await {
    ...
    async fn send_msg<'a,R>(&'a  self, spu_id: &'a SpuId, req_msg: &'a RequestMessage<R>) ->  Result<bool, ScServerError>
