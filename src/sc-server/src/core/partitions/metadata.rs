...
use metadata::partition::{PartitionSpec, PartitionStatus};
...
use crate::k8::default_convert_from_k8;
...
impl <S>From<((S, i32),Vec<i32>)> for PartitionKV where S: Into<String> {
...
impl PartitionLocalStore {
    ...
    pub fn replica_for_spu(&self, target_spu: &SpuId) -> Vec<Replica> {
        ...
        let msgs: Vec<Replica> = self.partition_spec_for_spu(target_spu)
            .into_iter()
            .map( |(replica_key,partition_spec)|  
                Replica::new(
                        replica_key,
                        partition_spec.leader,
                        partition_spec.replicas
                    )
            ).collect();
        debug!("{} computing replic msg for spuy: {}, msg: {}",self,target_spu,msgs.len());
        msgs
    }
...
impl <S>From<Vec<((S, i32), Vec<i32>)>> for PartitionLocalStore where S: Into<String>{
