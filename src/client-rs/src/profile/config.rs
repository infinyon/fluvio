...
pub mod test {
    ...
    fn test_default_path_arg() {
        assert_eq!(ConfigFile::default_file_path(Some("/user1/test".to_string())).expect("file"),
    ...
    fn test_config() {
        ...
        // test read & parse
        let mut conf_file = ConfigFile::load(Some("test-data/profiles/config.toml".to_owned())).expect("parse failed");
    ...
    fn test_tls_save() {
        ...
        config.save_to(temp_dir().join("inline.toml")).expect("save should succeed");

        config.mut_cluster(LOCAL_PROFILE).unwrap().tls = Some(TlsConfig::NoVerification);
        config.save_to(temp_dir().join("noverf.toml")).expect("save should succeed");
        ...
        config.save_to(temp_dir().join("file.toml")).expect("save should succeed");
    }
