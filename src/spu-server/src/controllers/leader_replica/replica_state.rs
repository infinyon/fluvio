...
use crate::controllers::follower_replica::PeerFileTopicReponse;
    ...
    fn add_follower_replica(&mut self, follower_ids: Vec<SpuId>) {
        ...
        for id in  follower_ids.into_iter().filter(|id| *id != leader_id) {
            if self.followers.contains_key(&id) {
                warn!("try to add existing follower id : {} to replica: {}, ignoring", id,self.replica_id);
            } else {
                trace!(
    ...
    S: ReplicaStorage,
{
        ...
        F: Into<FollowerOffsetUpdate>,
    {
        ...
        if follower_info.leo > leader_leo {
            warn!("offset leo: {} is greater than leader leo{} ",follower_info.leo,leader_leo);
    ...
    fn need_follower_updates(&self) -> Vec<(SpuId, FollowerReplicaInfo)> {
        ...
        self.followers.iter()
            .filter( | (_, follower_info) | follower_info.is_valid() && !follower_info.is_same(hw,leo))
            .map( |(follower_id,follower_info)| {
                debug!("replica: {}, follower: {} needs to be updated",self.replica_id,follower_id);
                trace!("follow: {} has different hw: {:#?}",follower_id,follower_info);
    ...
    fn as_lrs_request(&self) -> UpdateLrsRequest {
        ...
        let leader = (self.leader_id,self.storage.get_hw(),self.storage.get_leo()).into();
        let replicas = self.followers.iter()
            .map(|(follower_id,follower_info)| (*follower_id,follower_info.hw(),follower_info.leo()).into())
            .collect();
    ...
    pub async fn send_status_to_sc(&self,sc_sink: &ExclusiveKfSink) {
       let mut message = RequestMessage::new_request(self.as_lrs_request());
        message
            .get_mut_header()
            .set_client_id(format!("spu: {}, replica: {}",self.leader_id,self.replica_id));

        log_on_err!(sc_sink.send_request(&message).await);
        debug!("sent replica status to sc: {}, replica: {}",self.leader_id,self.replica_id);
    }
}
...
impl LeaderReplicaState<FileReplica> {
    ...
    pub async fn sync_follower(&self, sinks: &SinkPool<SpuId>,follower_id: SpuId,follower_info: &FollowerReplicaInfo) {
        ...
        if let Some(mut sink) = sinks.get_sink(&follower_id) {
            ...
            let mut topic_response = PeerFileTopicReponse::default();
    ...
    {
        self.storage.read_records_with_isolation(offset,isolation,partition_response).await
    }
...
mod test {
    ...
    fn test_follower_update() {
        ...
        let mut replica_state = LeaderReplicaState::new(("test", 1), 5000, mock_replica, vec![5001]);
        let follower_info = replica_state.followers.get(&5001).expect("follower should exists");
        ...
        assert_eq!(replica_state.update_follower_offsets((5001,10,10)),(true,Some((10,10).into())));

        
        // follower resync which sends same offset status, in this case no update but should trigger resync
        assert_eq!(replica_state.update_follower_offsets((5001,10,10)),(false,Some((10,10).into())));

        // finally follower updates the offset, this should trigger update but no resync
        assert_eq!(replica_state.update_follower_offsets((5001,20,10)),(true,None));

        // follower resync with same value, in this case no update and no resync
         assert_eq!(replica_state.update_follower_offsets((5001,20,10)),(false,None));
    }
    ...
    fn test_leader_update() {
        ...
        assert_eq!(replica_state.update_follower_offsets((5001,20,20)),(true,None));
        assert_eq!(replica_state.need_follower_updates().len(), 0);
