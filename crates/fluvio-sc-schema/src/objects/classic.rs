...
mod update_macro {
    ...
    // same as ClassicObjectApi but with limited type that can be updated
    macro_rules! ClassicUpdateApiEnum {
        ($api:ident) => {
            ...
            paste::paste! {
                ...
                impl Default for [<ClassicUpdateApi $api>] {
                    fn default() -> Self {
                        Self::Topic($api::<crate::topic::TopicSpec>::default())
                    }
                }

                impl [<ClassicUpdateApi $api>] {
                    fn type_string(&self) -> &'static str {
                        use fluvio_controlplane_metadata::core::Spec;
                        match self {
                            Self::Topic(_) => crate::topic::TopicSpec::LABEL,
                ...
                impl  fluvio_protocol::Encoder for [<ClassicUpdateApi $api>] {
                ...
                impl  fluvio_protocol::Decoder for [<ClassicUpdateApi $api>] {
    ...
    macro_rules! ClassicDecodingUpdate {
                ...
                impl  fluvio_protocol::Decoder for [<ObjectApi $api>] {
                        ...
                        T: fluvio_protocol::bytes::Buf
                    {
                        if version >= crate::objects::DYN_OBJ {
                            ...
                            use fluvio_protocol::Encoder;
                            let classic_obj = [<ClassicUpdateApi $api>]::decode_from(src, version)?;
                            tracing::debug!(ty = classic_obj.type_string(),version,"decoding as classical");
                            // reencode as DYN OBJECT to strip out type
                            self.0.set_buf(crate::objects::DYN_OBJ,classic_obj.type_string().to_owned(),classic_obj.as_bytes(crate::objects::DYN_OBJ)?.into());
                        }
    ...
    pub(crate) use ClassicUpdateApiEnum;
    pub(crate) use ClassicDecodingUpdate;
}
