...
pub async fn handle_delete_managed_connector<AC: AuthContext>(
    ...
    if let Ok(authorized) = auth_ctx
        .auth
        .allow_instance_action(ManagedConnectorSpec::OBJECT_TYPE, InstanceAction::Delete, &name)
        .await
    {
        if !authorized {
    ...
    let status = if auth_ctx
        ...
        .is_some()
    {
        if let Err(err) = auth_ctx.global_ctx.managed_connectors().delete(name.clone()).await {
