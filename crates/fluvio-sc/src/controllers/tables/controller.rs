...
impl TableController {
    pub fn start(ctx: SharedContext) {
        ...
        debug!("starting table controller");
        spawn(controller.dispatch_loop());
    }

    #[instrument(skip(self), name = "TableControllerLoop")]
    ...
    async fn inner_loop(&self) -> Result<(), IoError> {
        ...
        let mut table_listener = self.tables.change_listener();
        let _ = table_listener.wait_for_initial_sync().await;
        ...
        loop {
            ...
            select! {
                _ = table_listener.listen() => {
                    debug!("detected events in spu store");
                    table_listener.load_last();
    ...
    async fn sync_store(&self) -> Result<(), IoError> {
        ...
        //for updated_table in changes.into_iter() {
        //    let key = updated_table.key;
        //    let status = updated_table.status;
        //    debug!(id = updated_table.spec.id, status = %status, "updating table status");
        //    self.tables.update_status(key, status).await?;
        //}
