...
impl CheckStatus {
    ...
    pub fn pass<S: Into<String>>(msg: S) -> Self {
        Self::Pass(msg.into())
    }
}
...
pub trait ClusterCheck: Debug + 'static + Send + Sync {
    ...
    async fn perform_check(&self) -> Result<CheckStatus, ClusterCheckError>;
}

#[async_trait]
pub trait ClusterAutoFix: Debug + 'static + Send + Sync {
    /// Attempt to fix a recoverable error. return string
    async fn attempt_fix(&self) -> Result<String, ClusterAutoFixError>;
}

/// Check for loading
#[derive(Debug)]
pub struct ActiveKubernetesCluster;
...
#[derive(Debug)]
pub struct SysChartCheck {
    config: ChartConfig,
    platform_version: Version,
}

impl SysChartCheck {
    pub fn new(config: ChartConfig, platform_version: Version) -> Self {
        Self {
            config,
            platform_version,
        }
...
pub fn check_compare(first: &Box<dyn ClusterCheck>, second: &Box<dyn ClusterCheck>) -> Ordering {
