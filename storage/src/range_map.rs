...
impl SegmentList {
    ...
    // load segments
    pub async fn from_dir(option: &ConfigOption) -> Result<(SegmentList,Option<Offset>),StorageError>
     {
    ...
    pub fn add_segment(&mut self, segment: ReadSegment)  {
        ...
        self.min_base_offset = if self.min_base_offset < 0 { base_offset} else { min(self.min_base_offset,base_offset) };
...
mod tests {
    ...
    async fn create_segment(option: &ConfigOption,start: Offset, _offsets: Offset) -> Result<ReadSegment, StorageError> {
...
//        mut_segment.set_end_offset(offsets);
    ...
    async fn test_segment_read_many() -> Result<(),StorageError> {
        ...
        let segment2 = segments.get_segment(500).expect("should have segment at 500");
